...
def get_hierarchy_label(batch_labels, hierarchy_level):
    ...
    if max_hierarchy_level > 0:
        assert (batch_labels.ndim == 2) and batch_labels.shape[
            1] > max_hierarchy_level
...
def get_train_dataloader(dataset, batch_size, sampler, num_workers,
                         collate_fn):
...
def modelpath_creator(folder, basename, identifier, extension=".pth"):
    if identifier is None:
        ...
        return os.path.join(folder,
                            "%s_%s%s" % (basename, str(identifier), extension))
...
def load_dict_of_models(input_dict, suffix, folder, device, **kwargs):
    ...
    operate_on_dict_of_models(input_dict, suffix, folder, operation, "LOAD",
                              **kwargs)
...
def latest_version(folder, string_to_glob="trunk_*.pth", best=False):
    ...
    if items == []:
        return (0, None)
    model_regex = (
        regex_wrapper("best[0-9]+\.pth$") if best else regex_wrapper(
            "[0-9]+\.pth$")
    )
...
def assert_distance_type(obj, distance_type=None, **kwargs):
    ...
    for k, v in kwargs.items():
        assert getattr(obj.distance,
                       k) == v, "{} requires distance.{} to be {}".format(
            obj_name, k, v
        )
...
def set_ref_emb(embeddings, labels, ref_emb, ref_labels):
    if ref_emb is not None:
        if not torch.is_tensor(ref_labels):
            TypeError(
                "if ref_emb is given, then ref_labels must also be given")
...
def concatenate_indices_tuples(it1, it2):
    return tuple(
        [torch.cat([x, to_device(y, x)], dim=0) for x, y in zip(it1, it2)])
