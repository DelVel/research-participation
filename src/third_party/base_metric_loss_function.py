...
class BaseMetricLossFunction(
    ...
    def forward(
            self, embeddings, labels, indices_tuple=None, ref_emb=None,
            ref_labels=None
    ):
        """
        Args:
        ...
        ref_emb, ref_labels = c_f.set_ref_emb(embeddings, labels, ref_emb,
                                              ref_labels)
    ...
    def zero_loss(self):
        return {"losses": 0, "indices": None,
                "reduction_type": "already_reduced"}

    def zero_losses(self):
        return {loss_name: self.zero_loss() for loss_name in
                self.sub_loss_names()}
        ...
        for base_class in inspect.getmro(self.__class__):
            ...
            if base_class_name.endswith(mixin_keyword):
                ...
                if getattr(self, "{}_regularizer".format(descriptor)):
                    reg_names.extend(
                        base_class.regularization_loss_names(self))
        return reg_names
...
class MultipleLosses(torch.nn.Module):
    def __init__(self, losses, miners=None, weights=None):
        ...
        self.losses = (
            torch.nn.ModuleDict(
                losses) if self.is_dict else torch.nn.ModuleList(losses)
        )
    ...
    def forward(self, embeddings, labels, indices_tuple=None):
        ...
        total_loss = 0
        iterable = self.losses.items() if self.is_dict else enumerate(
            self.losses)
        for i, loss_func in iterable:
            ...
            total_loss += (
                    loss_func(embeddings, labels, curr_indices_tuple) *
                    self.weights[i]
            )
        return total_loss
    ...
    def assertions_if_not_none(self, x, match_all_keys):
        if x is not None:
            if self.is_dict:
                ...
                if match_all_keys:
                    assert sorted(list(x.keys())) == sorted(
